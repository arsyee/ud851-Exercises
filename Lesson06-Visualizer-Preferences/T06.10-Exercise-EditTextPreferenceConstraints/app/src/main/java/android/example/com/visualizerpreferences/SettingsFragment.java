...
public class SettingsFragment extends PreferenceFragmentCompat implements
    ...
    public boolean onPreferenceChange(Preference preference, Object newValue) {
        if (!preference.hasKey()) return errorToast("preference with no key has changed");
        if (preference.getKey().equals(getString(R.string.pref_size_key))) {
            if (!(newValue instanceof String)) return errorToast("new value is not a string");
            float value = 0;
            try {
                value = Float.parseFloat((String) newValue);
            } catch (NumberFormatException e) {
                return errorToast("new value is not a number");
            }
            if (value <= 0) return errorToast("size shall be greater than 0");
            if (value > 3) return errorToast("size cannot be higher than 3");
        }
        return true;
    }
