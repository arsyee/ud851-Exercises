    ...
    public Uri insert(@NonNull Uri uri, ContentValues values) {
        ...
        switch (sUriMatcher.match(uri)) {
            case TASKS:
                ...
                if (id == -1) {
                    throw new SQLiteException("Couldn't insert " + values + " into " + uri);
                }
                // COMPLETED (4) Set the value for the returnedUri and write the default case for unknown URI's
                resultUri = Uri.withAppendedPath(TaskContract.TaskEntry.CONTENT_URI, Long.toString(id));
                break;
            default:
                throw new UnsupportedOperationException("Cannot insert into " + uri.toString());
        }
